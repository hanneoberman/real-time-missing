...
- check calibration with prob or binary Y

- dgm formules

- figuren: blauw = 1269b0, rood = 7c1315, groen = 81c454
...
purrr::map(results[[1]][,-1], ~{rms::val.prob(p = .x, y = results[[1]]$Y_true)})
rms::val.prob(p = pred_list$Y_sub, y = predictions$Y_true)
rms::val.prob(p = predictions$Y_pred_log_mult, y = predictions$Y_true)

all_results <- test_run$results %>% map_dfr(~identity(.))
saveRDS(all_results, "results_10_sims.RDS")
all_results <- results_10_sims
purrr::map(all_results[,-1], ~{rms::val.prob(p = .x, y = all_results$Y_true)})
