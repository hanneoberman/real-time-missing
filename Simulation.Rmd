...
So instead, we'll create our own.^[Note: maybe add more variance-covariance matrices for sensitivity analyses later.]
...
```
Use the 10 predictors to calculate the outcome. Also include some interaction terms, a log transformation, and a spline to create a need for ML models; not just regression models.
...
```
The DGM model includes one cubic term and 9 interactions. 

The prevalence of the outcome is `r out$prevalence`. The C index/AUC of a simple logistic model is `r out$auc`. 
...
###: Q: Does it matter if we develop the log reg prediction model on the complete dataset or the imputed incomplete set?
...
- The prediction model is a flexible regression model (including non-linear effects using RCS (restricted qubic splines) with 4 knots and interaction terms). We also store the means and covariance of all predictor variables (which can be used to generate imputations).
