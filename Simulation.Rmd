...
- Rename 'true' model to spline model

- Add rf model after imputation
...
TODO: check c-stat [.6-.9]
TODO: report the betas
...
auc_log <-
    pROC::roc(
      Y ~ prob,
      data = cbind(devset, prob = predict(mod_log$mod, newdata = devset[,-1])),
      quiet = TRUE
    )$auc
```
...
Y_pred_mean <- suppressWarnings(pred_mean(imp_all, mod_log, mod_rf_imp))
Y_pred_draw <- suppressWarnings(pred_draw(imp_all, mod_log, mod_rf_imp))
```
