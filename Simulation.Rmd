...
# save data
saveRDS(trainset, file = "Data/trainset.RDS")
```
...
auc_true <-
    pROC::roc(
      Y ~ prob,
      data = cbind(devset, prob = predict(mod_true$mod, newdata = devset)),
      quiet = TRUE
    )$auc
...
auc_sub_2 <-
    pROC::roc(
      Y ~ prob,
      data = cbind(devset, prob = predict(mod_sub_2$mod, newdata =  devset)),
      quiet = TRUE
    )$auc
auc_sub_4 <-
    pROC::roc(
      Y ~ prob,
      data = cbind(devset, prob = predict(mod_sub_4$mod, newdata =  devset)),
      quiet = TRUE
    )$auc
auc_sub_6 <-
    pROC::roc(
      Y ~ prob,
      data = cbind(devset, prob = predict(mod_sub_6$mod, newdata =  devset)),
      quiet = TRUE
    )$auc
# rf models
...
auc_rf_imp <-
  pROC::roc(Y ~ prob,
            data = cbind(devset, prob = predict(mod_rf_imp$mod, data = devset)[["predictions"]]),
            quiet = TRUE)$auc
# pred_rf <-
