...
- (Only for strategy 1 and 3): Missing values are imputed by their conditional mean.^[What to do with categorical predictors?]
...
# TODO: add prediction using random forest
```
...
Add: **MAPE, RMSPE, mean absolute distance wrt real prob., think about AIC and calibration on population level while we're interested in individual level.**
...
# RMSE
(RMSE_obs <- purrr::map(pred_list, ~ calc_RMSE(pred = ., obs = pred_list[["Y"]])))
# (RMSE_prob <- purrr::map(pred_list, ~ calc_RMSE(pred = ., obs = pred_list[["Y_prob"]])))

# brier score
# (brier_obs <- purrr::map(pred_list, ~ calc_brier(pred = ., obs = valset[, "Y"])))
...
# plot calibration
plotdat <- data.frame(obs = rep(pred_list$Y_prob,4), pred = c(pred_list$Y_sub, pred_list$Y_imp_mean, pred_list$Y_imp_sing, pred_list$Y_imp_mult), method = c("surrogate", "cond.imp", "sing.imp", "mult.imp"))
...
# Vragen case study MIMIC-III: 

- Willen we een gevalideerd prediction model gebruiken inclusief parameterwaardes of alleen dezelfde varabelen gebruiken of een heel nieuw model fitten?

- Willen we de bestaande missingness gebruiken of missingness creëren (na imputateren van missingness of alleen in complete cases)?

- Willen het surrogate splits model trainen op incomplete populatiedata (omdat dat de enige methode is die dat kan), of net zoals de andere modellen op complete populatiedata en incomplete validatiedata?

# Overige vragen:

- Hoeveel keer willen we de simulatie herhalen? We hebben nu 1000 staan, maar in elke validatieset zitten ook al 20.000 cases. Zou minder dan genoeg zijn (want we evalueren dan eigenlijk een set van 20.000.000 individuele patienten)?

- Zouden we een laatste check over de methodes kunnen doen voordat we de HPC gaan 'misbruiken'? Bijv. of het aantal imputaties voor de 'draw from  conditional' methode inderdaad 50 moet zijn of we dat nog moeten tunen?
