...
# run this chuck only for development of new functions

# define DGM and missingness parameters
...
(RMSE <- purrr::map(pred_list, ~ calc_RMSE(pred = ., obs = pred_list[["Y_prob"]])))
rms::val.prob(p = Y_pred_sub, y = valset$Y_prob)
rms::val.prob(p = Y_pred_draw$mult, y = valset$Y)

# check if the object would be too big after replicating the simulation
# test <- rep(list(pred_list), 1000)
...
Y_pred_sub <- pred_sub(valset[,-2], mod_sub_2, mod_sub_4, mod_sub_6)
...
# impute the data for all md patterns
imp_all <- purrr::map(c(4,6,8), ~comp_cond(devset, valset, p = .x))
Y_pred_mean <- suppressWarnings(pred_mean(imp_all))
Y_pred_draw <- suppressWarnings(pred_draw(imp_all))
...
Add: **MAPE, RMSPE, mean absolute distance wrt real prob.**

Let op: Komt de kans richting het gemiddelde? Zie prestatie van 'methode' met alleen 0en als predicted prob invullen.

Iets met AIC en calibratie op populatieniveau. Ondanks dat we op indiv niveau geinteresserd zijn.
