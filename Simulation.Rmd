...
The C index/AUC of the logistic model (without interactions) is `r logistic$auc`, of the submodels it's `r round(auc4,2)`, `r round(auc6,2)`, and `r round(auc8,2)`, and of the random forest `r rf$auc`. This perfect accuracy of the rf on the training set should not be a problem, see https://towardsdatascience.com/one-common-misconception-about-random-forest-and-overfitting-47cae2e2c23b. But just to check, we fit these models on a new dev set.
...
auc_logistic <-
    pROC::roc(
      Y ~ prob,
      data = cbind(dataset, prob = predict(logistic$mod, newdata = dataset)),
      quiet = TRUE
    )$auc
# pattern mixture models
auc_pm4 <-
    pROC::roc(
      Y ~ prob,
      data = cbind(dataset, prob = predict(mod4, newdata =  dataset)),
      quiet = TRUE
    )$auc
auc_pm6 <-
    pROC::roc(
      Y ~ prob,
      data = cbind(dataset, prob = predict(mod6, newdata =  dataset)),
      quiet = TRUE
    )$auc
auc_pm8 <-
    pROC::roc(
      Y ~ prob,
      data = cbind(dataset, prob = predict(mod8, newdata =  dataset)),
      quiet = TRUE
    )$auc
# rf model
pred_dat <-
  predict(rf$mod, data = dataset) %>% .$predictions %>% cbind(dataset, prob = .)
...
- `r auc_pm4`, `r auc_pm6`, and `r auc_pm8` for the pattern mixture models;
...
We use a sample size of 2000 in the validation sets, to make sure there are at least 200 events in de val set, with the prevalence of ~0.15. 

### Q: I wrote down that we need 200 events in the val set, but also that we need 2000 events in the val set. Which one is correct?


```{r valset}
valset <- generate_sample(sample_size = 2000, Sigma, linear_betas, non_linear_betas, interaction = TRUE)
...
- Strategy 1: The prediction model is a flexible regression model **(including non-linear effects using RCS (restricted cubic splines) with 4 knots and interaction terms)**. We also store the means and covariance of all predictor variables (which can be used to generate imputations).
...
### Q: Should we indeed include interaction terms and splines in the regression model like the original study proposal? Or should we leave then out to make sure the performance of the different models is comparable?

### Q: Is there an example/article/repo for the surrogate splits random forest model?
