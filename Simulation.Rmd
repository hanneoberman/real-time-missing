...
rms::val.prob(p = predictions$Y_pred_sub, y = valset$Y)
rms::val.prob(p = predictions$Y_pred_log_mult, y = valset$Y)

# plot calibration
plotdat <- data.frame(obs = rep(pred_list$Y_obs,4), pred = c(pred_list$Y_sub, pred_list$Y_imp_mean, pred_list$Y_imp_sing, pred_list$Y_imp_mult), method = c("submodel", "cond.imp", "sing.imp", "mult.imp"))
