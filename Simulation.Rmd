...
auc_pm8 <-
    pROC::roc(
      Y ~ prob,
      data = cbind(devset, prob = predict(mod8, newdata =  devset)),
      quiet = TRUE
    )$auc
# rf model
pred_dat <-
...
- Strategy 1: The prediction model is a flexible regression model including non-linear effects using RCS (restricted cubic splines) with 3 knots. We also store the means and covariance of all predictor variables (which can be used to generate imputations).
