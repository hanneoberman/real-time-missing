...
#pred <- predict(mod, data=dat)
rf <- fit_rf(dat)
# saveRDS(rf, file = "Data/rf_model.RDS")

# why is the auc 1? test on new data
dataset <- generate_sample(n, Sigma, interaction = TRUE)
...
(auc_logistic <-
    pROC::roc(Y ~ prob,
              data = cbind(dataset, prob = predict(logistic$mod)),
              quiet = TRUE)$auc)

```


The prevalence of the outcome is `r mean(dat$Y)`. The C index/AUC of the logistic model (without interactions) is `r logistic$auc`, and of the random forest `r rf$auc`. 

Voor PMKS code: https://core.ac.uk/download/pdf/216055043.pdf
...
set.seed(1)
MAR_types <- rep(c("LEFT", "MID", "TAIL", "RIGHT"), 3) #sample(c("LEFT", "MID", "TAIL", "RIGHT"), size = 12, replace = TRUE)
...
## No! 4, 6 of 8 pred ontbrekend per respondent
