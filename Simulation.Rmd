...
trainset <-
  generate_sample(n = 10000, varcov, linear_betas, non_linear_betas, interaction = TRUE)

# what do the data look like?
glimpse(trainset)
```

The aim is to have a prevalence of about 0.15. In this dev set, the prevalence of the outcome is `r mean(trainset$Y)`. 
...
# logistic model
mod_true <- fit_true(trainset)

# pattern mixture modtrainset
mod_sub_6 <- fit_sub(trainset, Y ~ X1 + X2 + X3 + X4 + X5 + X6) 
mod_sub_4 <- fit_sub(trainset, Y ~ X1 + X2 + X3 + X4) 
mod_sub_2 <- fit_sub(trainset, Y ~ X1 + X2) 

# random forest models
mod_rf_imp <- fit_rf_imp(trainset)
# mod_rf_sur <- moreparty::fastcforest(Y~., data=dat)
# mod_rf_sur <- fit_fr_sub(dat)
...
devset <-
  generate_sample(n, varcov, linear_betas, non_linear_betas, interaction = TRUE)

# logistics model
auc_true <-
    pROC::roc(
      Y ~ prob,
      data = cbind(devset, prob = predict(mod_true$mod, newdata = devset[,-1])),
      quiet = TRUE
    )$auc
...
auc_sub_2 <-
    pROC::roc(
      Y ~ prob,
      data = cbind(devset, prob = predict(mod_sub_2$mod, newdata =  devset[,-1])),
      quiet = TRUE
    )$auc
auc_sub_4 <-
    pROC::roc(
      Y ~ prob,
      data = cbind(devset, prob = predict(mod_sub_4$mod, newdata =  devset[,-1])),
      quiet = TRUE
    )$auc
auc_sub_6 <-
    pROC::roc(
      Y ~ prob,
      data = cbind(devset, prob = predict(mod_sub_6$mod, newdata =  devset[,-1])),
      quiet = TRUE
    )$auc
# rf models
# pred_rf_imp <- predict(mod_rf_imp$mod, data = devset) %>% .$predictions %>% cbind(devset, prob = .)
auc_rf_imp <-
  pROC::roc(Y ~ prob,
            data = cbind(devset, prob = predict(mod_rf_imp$mod, data = devset[,-1])[["predictions"]]),
            quiet = TRUE)$auc
# pred_rf <-
...
- `r auc_sub_6`, `r auc_sub_4`, and `r auc_sub_2` for the pattern mixture models;
...
valset <- generate_sample(sample_size = 20000, varcov, linear_betas, non_linear_betas, interaction = TRUE)
```
...
testset <- create_miss(valset, miss_par)
# testset <- valset[,-1] %>% 
#   mice::ampute(mech = "MAR", prop = 0.999, patterns = miss_pat, type = MAR_types) %>% 
#   .$amp 
...
- Strategy 1: The prediction model is a flexible regression model including non-linear effects using RCS (restricted cubic splines) with 3 knots. We also store the means and covariance of all predictor variables (using package `condmvnorm`, which can be used to generate imputations).
...
testset$nob <- rowSums(!is.na(testset)) #add the number of observed variables
testsplit <- split(testset, testset$nob)
Y_pred_mod2 <- predict(mod_sub_2$mod, newdata = testsplit$`2`)
