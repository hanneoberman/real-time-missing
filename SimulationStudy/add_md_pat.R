...
results_md_pat <-
  purrr::map(1:1000, function(.x) {
    cbind(pmis = datasets[[.x]][["valset"]][["p_miss"]], results[[.x]])
  })
...
cali <- purrr::map(results_md_pat, function(.i) {
  split(results_md_pat[[.i]], ~ pmis) |>
    purrr::map(., function(.p) {
      purrr::map_dfr(meth_lab, ~ {
        lm(.i$Y_prob ~ .i[, .x])$coefficients %>% setNames(c("Intercept", "Slope"))
      }) %>% cbind(Method = meth_lab, .)
    })
  rmse <- purrr::map_dfr(results_md_pat, function(.i) {
    purrr::map_dfc(.i[, -c(1:2)], ~ {
      sqrt(mean((.x - .i$Y_prob) ^ 2))
    })
  }) %>%
    tidyr::pivot_longer(cols = everything(),
                        names_to = "Method",
                        values_to = "RMSE") #%>%
  brier <- purrr::map_dfr(results_md_pat, function(.i) {
    purrr::map_dfc(.i[, -c(1:2)], ~ {
      mean((.x - .i$Y_true) ^ 2)
    })
  }) %>%
    tidyr::pivot_longer(cols = everything(),
                        names_to = "Method",
                        values_to = "Brier") # %>%
  auc <- purrr::map_dfr(results_md_pat, function(.i) {
    purrr::map_dfc(.i[, -c(1:2)], ~ {
    ...
    tidyr::pivot_longer(cols = everything(),
                        names_to = "Method",
                        values_to = "AUC") #%>%
  mae <- purrr::map_dfr(results_md_pat, function(.i) {
    purrr::map_dfc(.i[, -c(1:2)], ~ {
      abs(.x - .i$Y_prob) %>% mean()
    })
  }) %>%
  ...
  performance_md_pat <- cali %>%
    ...
    mutate(
      ...
      Model = case_when(stringr::str_detect(Method, "FLR") ~ "FLR", TRUE ~
                          "RF"),
